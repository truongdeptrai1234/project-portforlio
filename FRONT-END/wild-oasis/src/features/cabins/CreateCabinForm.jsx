...
function CreateCabinForm({ editOption, editItemData = null }) {
  const { register, handleSubmit, reset, getValues, formState } = useForm({
    defaultValues: { ...editItemData },
  });
  ...
  function onSubmit(data) {
    if (!data?.image[0]?.name && editItemData) {
      data = { ...data, image: editItemData.image };
    }
    if (editItemData) updateCabin(data, { onSuccess: () => reset() });
    if (!editItemData) createNewCabin(data, { onSuccess: () => reset() });
  }
  function onError() {
    console.log(errors);
  }
